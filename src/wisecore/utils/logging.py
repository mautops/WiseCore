# -*- coding: utf-8 -*-
"""Logging setup: console output and optional file handler."""
...
def _enable_windows_ansi() -> None:
    ...
    try:
        ...
        # Silently ignore: ANSI support is optional, failure doesn't affect
        # core functionality. Older Windows versions may not support it.
        pass
...
class SuppressPathAccessLogFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        try:
            ...
            # On error, allow the log through - filtering is optional
            return True


def setup_logger(level: int | str = logging.INFO):
    """Configure logging to only output from this package (wisecore), not deps."""
    ...
    # Only attach handler to our namespace so only wisecore.* logs are printed.
...
def add_wisecore_file_handler(log_path: Path) -> None:
    """Add a file handler to the wisecore logger for daemon logs.
    ...
    Args:
        log_path: Path to the log file (e.g. WORKING_DIR / "wisecore.log").
    """
