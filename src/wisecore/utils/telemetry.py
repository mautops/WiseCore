...
TELEMETRY_ENDPOINT = "https://wisecore-telemetry-xissagieap.cn-hangzhou.fcapp.run"
...
def _safe_get(func: Callable[[], str], default: str = "unknown") -> str:
    ...
    try:
        ...
        logger.debug("Failed to get value: %s", e)
        return default


def _detect_install_method() -> str:
    """Detect how the application was installed based on environment signals."""
    import os

    if os.environ.get("RUNNING_IN_CONTAINER", "").lower() in (
        "1",
        "true",
        "yes",
    ):
        return "docker"
    if os.environ.get("DESKTOP_APP", "").lower() in (
        "1",
        "true",
        "yes",
    ):
...
def get_system_info() -> dict[str, Any]:
    ...
    - install_method: How the application was installed (docker/desktop/pip)
    - os: Operating system (Windows/Darwin/Linux)
    - os_version: OS version string
    - python_version: Python version running wisecore (major.minor)
    - architecture: CPU architecture (x86_64/arm64/etc)
    - has_gpu: GPU availability detection
    """
    from ..__version__ import __version__ as wisecore_ver

    info = {
        "install_id": str(uuid.uuid4()),
        "wisecore_version": _safe_get(lambda: wisecore_ver, "unknown"),
...
def _get_current_version() -> str:
    ...
    try:
        ...
        logger.debug("Failed to get current version: %s", e)
        return "unknown"
...
def has_telemetry_been_collected(working_dir: Path) -> bool:
    ...
    Re-triggers collection when upgraded (or downgraded) to a version
    ...
    try:
        ...
        # v1.1 compat: single wisecore_version field
        return marker_data.get("wisecore_version", "") == current
    except Exception as e:
        logger.debug("Failed to read telemetry marker: %s", e)
        return False
...
def is_telemetry_opted_out(working_dir: Path) -> bool:
    ...
    try:
        ...
        logger.debug("Failed to check telemetry opt-out: %s", e)
        return False
        ...
        if marker_file.exists():
            try:
                ...
                if not collected_versions:
                    old_ver = old_data.get("wisecore_version", "")
                    if old_ver:
                        collected_versions = [old_ver]
            except Exception as e:
                logger.debug("Failed to migrate telemetry marker: %s", e)
