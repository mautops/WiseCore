...
from wisecore.providers.multimodal_prober import (
    ProbeResult,
    _PROBE_IMAGE_B64,
    _PROBE_VIDEO_URL,
    _is_media_keyword_error,
)
from wisecore.providers.provider import ModelInfo, Provider
...
class GeminiProvider(Provider):
    ...
    async def check_connection(self, timeout: float = 10) -> tuple[bool, str]:
        ...
        try:
            ...
            logger.debug("Unknown exception connecting to Gemini API: %s", e)
            return (
                False,
    ...
    async def fetch_models(self, timeout: float = 10) -> List[ModelInfo]:
        ...
        try:
            ...
            logger.debug("Failed to fetch Gemini models: %s", e)
            return []
    ...
    async def check_model_connection(
        ...
        try:
            ...
            logger.debug("Unknown exception connecting to Gemini model %s: %s", model_id, e)
            return (
                False,
