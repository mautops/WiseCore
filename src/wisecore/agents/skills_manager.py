...
def ensure_skills_initialized(workspace_dir: Path) -> None:
    ...
    Skills should be configured via `wisecore init` or
    `wisecore skills config`.
    """
    active_skills = get_active_skills_dir(workspace_dir)
    ...
    if not active_skills.exists() or not available:
        logger.warning(
            ...
            "Run 'wisecore init' or 'wisecore skills config' "
            "to configure skills.",
        )
    else:
        logger.debug(
...
def _import_skill_dir(
    ...
    try:
        ...
        logger.debug("Imported skill '%s' from zip.", skill_name)
        return True
    except Exception as e:
        logger.error(
...
class SkillService:
    ...
    def import_from_zip(
        ...
        try:
            tmp_dir = Path(
                tempfile.mkdtemp(prefix="wisecore_skill_upload_"),
            )
