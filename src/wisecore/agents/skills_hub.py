...
def _hub_http_timeout() -> float:
    raw = os.environ.get("SKILLS_HUB_HTTP_TIMEOUT", "15")
    try:
        return max(3.0, float(raw))
    except Exception as e:
        logger.debug("Invalid SKILLS_HUB_HTTP_TIMEOUT: %s", e)
        return 15.0


def _hub_http_retries() -> int:
    raw = os.environ.get("SKILLS_HUB_HTTP_RETRIES", "3")
    try:
        return max(0, int(raw))
    except Exception as e:
        logger.debug("Invalid SKILLS_HUB_HTTP_RETRIES: %s", e)
        return 3


def _hub_http_backoff_base() -> float:
    raw = os.environ.get("SKILLS_HUB_HTTP_BACKOFF_BASE", "0.8")
    try:
        return max(0.1, float(raw))
    except Exception as e:
        logger.debug("Invalid SKILLS_HUB_HTTP_BACKOFF_BASE: %s", e)
        return 0.8


def _hub_http_backoff_cap() -> float:
    raw = os.environ.get("SKILLS_HUB_HTTP_BACKOFF_CAP", "6")
    try:
        return max(0.5, float(raw))
    except Exception as e:
        logger.debug("Invalid SKILLS_HUB_HTTP_BACKOFF_CAP: %s", e)
        return 6.0
...
def _ensure_not_cancelled() -> None:
    ...
    try:
        ...
        # Ignore checker failures and continue - cancellation is optional.
        logger.debug("Cancel checker failed: %s", e)
        return
...
def _hub_base_url() -> str:
    return os.environ.get("SKILLS_HUB_BASE_URL", "https://clawhub.ai")


def _hub_search_path() -> str:
    return os.environ.get(
        "SKILLS_HUB_SEARCH_PATH",
        "/api/v1/search",
    )
...
def _build_request(full_url: str, accept: str) -> Request:
    req = Request(
        ...
        headers={
            "Accept": accept,
            "User-Agent": "wisecore-skills-hub/1.0",
        },
    )
...
def _github_repo_exists(owner: str, repo: str) -> bool:
    ...
    try:
        ...
        logger.debug("GitHub repo check failed for %s/%s: %s", owner, repo, e)
        return False
...
def _github_read_file(entry: dict[str, Any]) -> str:
    ...
    if isinstance(content, str) and content:
        try:
            ...
            logger.debug("Failed to decode GitHub file content: %s", e)
            pass
...
def _fetch_bundle_from_github_url(
    ...
    try:
        ...
        logger.debug(
            "Failed to get default branch for %s/%s: %s", owner, repo, e
        )
        pass
