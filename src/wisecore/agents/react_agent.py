# -*- coding: utf-8 -*-
"""Core Agent - Main agent implementation.

This module provides the main CoreAgent class built on ReActAgent,
...
class CoreAgent(ToolGuardMixin, ReActAgent):
    """Core Agent with integrated tools, skills, and memory management.
    ...
    Python's MRO: CoreAgent → ToolGuardMixin → ReActAgent.  If you
    ...
    **must** call ``super()._acting(...)`` / ``super()._reasoning(...)``
    ...
    def _wrap_shell_tool(self, tool_func):
        """Wrap shell tool to inject ``request_context`` into subprocess env.

        Only ``cli_access_token`` is injected as ``CLI_ACCESS_TOKEN``.
        The value is never persisted across tool calls.
        """
        ...
        async def wrapper(command: str, **kwargs):
            for pattern in (
                ...
                r"\bdeclare\s+-x\s+\w+",
                r"\benv\s+\w+=",
                r"(?i)\b(printenv|env|set)\b.*\bCLI_ACCESS_TOKEN\b",
                r"(?i)\b(echo|printf)\b[^\n]*\bCLI_ACCESS_TOKEN\b",
            ):
                if re.search(pattern, command):
                    logger.warning(
                        "Blocked env manipulation: %s", command[:100]
                    )
            ...
            cli_token = self._request_context.get("cli_access_token")
            if cli_token is not None:
                env["CLI_ACCESS_TOKEN"] = str(cli_token)

            if cli_token and not command.startswith("bash -c "):
                escaped_cmd = command.replace('"', '\\"')
                command = f'bash -c "{escaped_cmd}; unset CLI_ACCESS_TOKEN"'
    ...
    def _create_toolkit(
        ...
        for tool_name, tool_func in tool_functions.items():
            ...
            wrapped_func = (
                self._wrap_shell_tool(tool_func)
    ...
    def _rebuild_mcp_client(client: Any) -> Any | None:
        ...
        rebuild_info = getattr(client, "_wisecore_rebuild_info", None)
        ...
        try:
            if transport == "stdio":
                ...
                setattr(rebuilt_client, "_wisecore_rebuild_info", rebuild_info)
                return rebuilt_client
    ...
    def _strip_tool_use_from_msg(msg: Msg) -> Msg:
        ...
        if isinstance(msg.content, str):
            msg.content += CoreAgent._ROUND_END_NOTICE
            return msg
        ...
        filtered.append({"type": "text", "text": CoreAgent._ROUND_END_NOTICE})
        msg.content = filtered
        return msg
    ...
    async def reply(
        ...
        if query is not None and self.command_handler.is_command(query):
            logger.debug("Received command: %s", query)
            msg = await self.command_handler.handle_command(query)
            await self.print(msg)
            return msg

        # Normal message processing
        logger.debug("CoreAgent.reply: max_iters=%s", self.max_iters)
