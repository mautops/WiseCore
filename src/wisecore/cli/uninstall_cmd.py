# -*- coding: utf-8 -*-
"""wisecore uninstall — remove the Wisecore environment and CLI wrapper."""
...
def _remove_path_entry(profile: Path) -> bool:
    """
    Remove Wisecore PATH lines from a shell profile. Returns True if changed.
    """
    if not profile.is_file():
        return False

    text = profile.read_text()
    # Remove the "# Wisecore" comment line and the export PATH line
    cleaned = re.sub(
        r"\n?# Wisecore\nexport PATH=\"\$HOME/\.wisecore/bin:\$PATH\"\n?",
        "\n",
        text,
    )
    if cleaned == text:
...
def uninstall_cmd(purge: bool, yes: bool) -> None:
    """Remove Wisecore environment, CLI wrapper, and shell PATH entries."""
    wd = WORKING_DIR

    if purge:
        click.echo(f"This will remove ALL Wisecore data in {wd}")
    else:
        click.echo(
            "This will remove the Wisecore Python environment and CLI wrapper.",
        )
    ...
    click.echo("")
    click.echo("Wisecore uninstalled. Please restart your terminal.")
