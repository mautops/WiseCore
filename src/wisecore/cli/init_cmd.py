...
Wisecore is a personal assistant that runs in your own environment. It can connect to
...
If multiple people can message the same Wisecore instance with tools enabled, they
share the same delegated authority (files, commands, secrets the agent can use).

If you are not comfortable with access control and hardening, do not run Wisecore with
...
TELEMETRY_INFO = """
Help improve Wisecore by sharing anonymous usage data!

We collect only:
• Wisecore version (e.g., 0.0.7)
...
This helps us understand Wisecore's usage environment and prioritize improvements.
"""
...
def _echo_telemetry_info_box() -> None:
    ...
    console.print(
        Panel(
            ...
            title="[bold]📊 Help improve Wisecore[/bold]",
            border_style="blue",
        ),
    )
