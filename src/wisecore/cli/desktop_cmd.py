# -*- coding: utf-8 -*-
"""CLI command: run Wisecore app on a free port in a native webview window."""
...
def _stream_reader(in_stream, out_stream) -> None:
    ...
    try:
        ...
        # Best-effort stream draining: ignore errors (stream may be closed)
        pass
    finally:
        try:
            in_stream.close()
        except Exception:
            # Best-effort cleanup: ignore errors (stream may already be closed)
            pass
...
def desktop_cmd(
    ...
    """Run Wisecore app on an auto-selected free port in a webview window.
    ...
    window without conflicting with an existing Wisecore app instance.
    """
    ...
    click.echo(f"Starting Wisecore app on {url} (port {port})")
