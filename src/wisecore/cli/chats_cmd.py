...
def chats_group() -> None:
    ...
    \b
    Common examples:
      wisecore chats list                    # List all chats
      wisecore chats list --user-id alice    # Filter by user
      wisecore chats get <chat_id>           # View details
      wisecore chats create --session-id s1 --user-id u1
      wisecore chats delete <chat_id>        # Delete a chat
    """
...
def list_chats(
    ...
    Examples:
      wisecore chats list
      wisecore chats list --user-id alice
      wisecore chats list --channel discord
      wisecore chats list --user-id alice --channel discord
    """
...
def get_chat(
    ...
    \b
    CHAT_ID  Chat UUID, obtainable via `wisecore chats list`.

    \b
    Examples:
      wisecore chats get 823845fe-dd13-43c2-ab8b-d05870602fd8
    """
    base_url = resolve_base_url(ctx, base_url)
    with client(base_url) as c:
        headers = {"X-Agent-Id": agent_id}
...
def create_chat(
    ...
    Inline creation examples:
      wisecore chats create --session-id "discord:alice" \\
    ...
    JSON file creation example:
      wisecore chats create -f chat.json
    """
    base_url = resolve_base_url(ctx, base_url)
...
def update_chat(
    ...
    \b
    Examples:
      wisecore chats update <chat_id> --name "Renamed Chat"
    """
    base_url = resolve_base_url(ctx, base_url)
    headers = {"X-Agent-Id": agent_id}
...
def delete_chat(
    ...
    \b
    Examples:
      wisecore chats delete 823845fe-dd13-43c2-ab8b-d05870602fd8
    """
    base_url = resolve_base_url(ctx, base_url)
    with client(base_url) as c:
        headers = {"X-Agent-Id": agent_id}
