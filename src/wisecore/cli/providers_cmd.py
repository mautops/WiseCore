...
def configure_llm_slot_interactive(*, use_defaults: bool = False) -> None:
    ...
    if not eligible:
        if use_defaults:
            click.echo(
                "No LLM provider configured. Run 'wisecore models config' "
                "to configure later.",
            )
            return
        click.echo(
    ...
    if not model and use_defaults:
        click.echo(
            ...
            "Run 'wisecore models config' to set one.",
        )
        return
    try:
    ...
    for defn in _all_provider_objects(manager):
        ...
        if defn.is_local:
            ...
            if all_models:
                ...
                click.echo("  Use 'wisecore models download' to add models.")
        else:
...
def add_provider_cmd(
    ...
    click.echo(
        "  Run 'wisecore models add-model' to add models, "
        "then 'wisecore models config-key' to set the API key.",
    )
...
def download_cmd(
    ...
    \b
    Examples:
      wisecore models download TheBloke/Mistral-7B-Instruct-v0.2-GGUF
      wisecore models download TheBloke/Mistral-7B-Instruct-v0.2-GGUF \\
          -f mistral-7b-instruct-v0.2.Q4_K_M.gguf
      wisecore models download Qwen/Qwen2-0.5B-Instruct-GGUF --source modelscope
    """
    try:
        ...
        click.echo(
            click.style(
                ...
                "Install with: pip install 'wisecore[local]'",
                fg="red",
            ),
        )
        raise SystemExit(1) from exc
    ...
    click.echo(
        ...
        f"  wisecore models set-llm  (select '{backend}' provider)",
    )
...
def list_local_cmd(backend: str | None) -> None:
    ...
    if not models:
        ...
        click.echo("Use 'wisecore models download <repo_id>' to download one.")
        return
...
def ollama_pull_cmd(model_name: str) -> None:
    ...
    \b
    Examples:
      wisecore models ollama-pull mistral:7b
      wisecore models ollama-pull qwen2.5:3b
    """
...
def ollama_list_cmd() -> None:
    ...
    if not models:
        ...
        click.echo("Use 'wisecore models ollama-pull <model>' to download one.")
        return
...
def ollama_remove_cmd(model_name: str, yes: bool) -> None:
    ...
    \b
    Examples:
      wisecore models ollama-remove mistral:7b
      wisecore models ollama-remove qwen2.5:3b -y
    """
    if not yes:
