...
def _matches_wisecore_cli_command(command: str, *subcommands: str) -> bool:
    """Return whether command line looks like a Wisecore CLI invocation."""
    lowered = f" {command.lower()}"
    return any(
        pattern in lowered
        for subcommand in subcommands
        for pattern in (
            f" -m wisecore {subcommand}",
            f" wisecore {subcommand}",
            f"__main__.py {subcommand}",
            f'wisecore.exe" {subcommand}',
            f"wisecore.exe {subcommand}",
        )
    )


def _is_wisecore_service_command(command: str) -> bool:
    """Return whether the command line looks like a local Wisecore app."""
    return _matches_wisecore_cli_command(command, "app")


def _is_wisecore_wrapper_process(name: str, command: str) -> bool:
    """Return whether the process looks like a Wisecore CLI wrapper."""
    lowered_name = name.lower().removesuffix(".exe")
    return lowered_name == "wisecore" or _matches_wisecore_cli_command(
        command,
        "app",
        "desktop",
    )
...
def _candidate_hosts(host: str | None) -> list[str]:
    """Return host variants that can reach a local Wisecore service."""
    if not host:
        return []
