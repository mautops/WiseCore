...
def _load_config_tool_guard():
    ...
    try:
        ...
        logger.debug("Failed to load tool guard config: %s", e)
        return None
...
def resolve_guarded_tools(
    ...
    raw = os.environ.get("TOOL_GUARD_TOOLS")
    if raw is not None:
...
def resolve_denied_tools(
    ...
    Priority:
    1) constructor-provided ``user_defined``
    2) ``TOOL_GUARD_DENIED_TOOLS`` env var (comma-separated)
    ...
    raw = os.environ.get("TOOL_GUARD_DENIED_TOOLS")
    if raw is not None:
