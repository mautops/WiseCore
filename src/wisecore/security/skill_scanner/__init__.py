...
Quick start::
    ...
    from wisecore.security.skill_scanner import SkillScanner
...
def _get_scan_mode(cfg: Any = None) -> str:
    ...
    Priority: env ``SKILL_SCAN_MODE`` > config > default ``warn``.
    """
    env = os.environ.get("SKILL_SCAN_MODE")
    if env is not None:
        val = env.lower().strip()
...
def _get_blocked_history_path() -> Path:
    try:
        ...
        return Path.home() / ".wisecore" / _BLOCKED_HISTORY_FILE
