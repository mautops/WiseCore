...
def _normalize_working_dir_bound_paths(data: object) -> object:
    ...
    This keeps WORKING_DIR effective even if user config files contain
    older hard-coded paths like "~/.wisecore/media" or
    "/Users/x/.wisecore/workspaces/...".
    Only rewrites known working-dir-bound keys.
    """
    legacy_root_tilde = "~/.wisecore"
...
def get_available_channels() -> Tuple[str, ...]:
    ...
    wisecore.channels), filtered by ENABLED_CHANNELS or
    DISABLED_CHANNELS when set.

    * ENABLED_CHANNELS — whitelist (only these channels are active).
    * DISABLED_CHANNELS — blacklist (all channels *except* these).
    * If both are set, ENABLED_CHANNELS takes precedence.
...
def is_running_in_container() -> bool:
    ...
    Prefer env RUNNING_IN_CONTAINER (1/true/yes) at call time so
...
def get_heartbeat_config(agent_id: Optional[str] = None) -> HeartbeatConfig:
    ...
    if agent_id is not None:
        try:
            ...
            logger.debug(
                "Failed to load heartbeat config for agent %s: %s",
                agent_id,
                e,
            )
...
def update_last_dispatch(
    ...
    if agent_id is not None:
        try:
            ...
            logger.debug(
                "Failed to update last_dispatch for agent %s: %s",
                agent_id,
                e,
            )
            pass
