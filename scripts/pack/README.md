# Wisecore Desktop packaging scripts
...
# Output: dist/Wisecore-Setup-<version>.exe
# Creates two launchers:
#   - Wisecore Desktop.vbs (silent, no console window)
#   - Wisecore Desktop (Debug).bat (shows console for troubleshooting)
...
APP_ENV="$(pwd)/dist/Wisecore.app/Contents/Resources/env"
PYTHONPATH= PYTHONHOME="$APP_ENV" "$APP_ENV/bin/python" -m wisecore desktop
```
...
When you **double-click** the .app and nothing appears, the launcher writes stderr/stdout to `~/.wisecore/desktop.log`. Inspect that file for errors.
...
When users download the Wisecore macOS app (e.g. from Releases) as a `.app` (in a zip), macOS may show: *"Apple cannot verify that 'Wisecore' contains no malicious software"*. The app is not notarized. They can still open it as follows:

- **Right-click to open (recommended)**
  Right-click (or Control+click) the Wisecore app → **Open** → in the dialog click **Open** again. Gatekeeper will allow it; after that double-click works as usual.

- **Allow in System Settings**
  If still blocked, go to **System Settings → Privacy & Security**, find the message like *"Wisecore was blocked because it is from an unidentified developer"*, and click **Open Anyway** or **Allow**.

- **Remove quarantine attribute (not recommended for most users)**
  In Terminal: `xattr -cr /Applications/Wisecore.app` (or the path to the `.app` after unzipping). This clears the download quarantine flag; less safe than right-click → Open.
...
| File | Description |
|------|-------------|
| `build_common.py` | Create temporary conda env, install `wisecore[full]` from a wheel, conda-pack; produces archive. |
| `build_macos.sh` | One-click: build wheel → build_common → unpack into Wisecore.app; optional zip. |
| `build_win.ps1` | One-click: build wheel → build_common → unpack → create VBS/BAT launchers → makensis installer. |
| `wisecore_desktop.nsi` | NSIS script: pack `dist/win-unpacked`, add icons, and create shortcuts. |
