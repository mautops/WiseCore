...
$Archive = Join-Path $Dist "wisecore-env.zip"
$Unpacked = Join-Path $Dist "win-unpacked"
$NsiPath = Join-Path $PackDir "wisecore_desktop.nsi"
...
$VersionFile = Join-Path $RepoRoot "src\wisecore\__version__.py"
$CurrentVersion = ""
...
$RunWheelBuild = $true
if ($CurrentVersion) {
  $wheelGlob = Join-Path $Dist "wisecore-$CurrentVersion-*.whl"
  ...
  if ($existingWheels.Count -gt 0) {
    ...
    $oldWheels = Get-ChildItem -Path (Join-Path $Dist "wisecore-*.whl") -ErrorAction SilentlyContinue
...
$LauncherBat = Join-Path $EnvRoot "Wisecore Desktop.bat"
@"
@echo off
cd /d "%~dp0"
...
REM Log level: env var WISECORE_LOG_LEVEL or default to "info"
if not defined WISECORE_LOG_LEVEL set "WISECORE_LOG_LEVEL=info"

REM Set SSL certificate paths for packaged environment
REM Use temp file to avoid for /f blocking issue in bat scripts
set "CERT_TMP=%TEMP%\wisecore_cert_%RANDOM%.txt"
...
if not exist "%USERPROFILE%\.wisecore\config.json" (
  "%~dp0python.exe" -u -m wisecore init --defaults --accept-security
)
"%~dp0python.exe" -u -m wisecore desktop --log-level %WISECORE_LOG_LEVEL%
"@ | Set-Content -Path $LauncherBat -Encoding ASCII

# Debug launcher .bat (shows console)
$DebugBat = Join-Path $EnvRoot "Wisecore Desktop (Debug).bat"
@"
@echo off
cd /d "%~dp0"
...
REM Debug mode: use debug log level by default (can override with WISECORE_LOG_LEVEL)
if not defined WISECORE_LOG_LEVEL set "WISECORE_LOG_LEVEL=debug"
...
echo ====================================
echo Wisecore Desktop - Debug Mode
echo ====================================
echo Working Directory: %cd%
echo Python: "%~dp0python.exe"
echo PATH: %PATH%
echo Log Level: %WISECORE_LOG_LEVEL%
...
echo [Launch] Starting Wisecore Desktop with log-level=%WISECORE_LOG_LEVEL%...
echo Press Ctrl+C to stop
echo.
"%~dp0python.exe" -u -m wisecore desktop --log-level %WISECORE_LOG_LEVEL%
echo.
echo [Exit] Wisecore Desktop closed
pause
"@ | Set-Content -Path $DebugBat -Encoding ASCII

# VBScript launcher (no console window)
$LauncherVbs = Join-Path $EnvRoot "Wisecore Desktop.vbs"
@"
Set WshShell = CreateObject("WScript.Shell")
batPath = CreateObject("Scripting.FileSystemObject").GetParentFolderName(WScript.ScriptFullName) & "\Wisecore Desktop.bat"
...
if (-not $Version) {
  ...
  try {
    $Version = (& (Join-Path $EnvRoot "python.exe") -c "from importlib.metadata import version; print(version('wisecore'))" 2>&1) -replace '\s+$', ''
...
Write-Host "[build_win] WISECORE_VERSION=$Version OUTPUT_EXE will be under $Dist"
$OutInstaller = Join-Path (Join-Path $RepoRoot $Dist) "Wisecore-Setup-$Version.exe"
